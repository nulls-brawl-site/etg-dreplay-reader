...
OUT="$BUILD/etg-dreplay-reader.dex"

ANDROID_JAR="${ANDROID_JAR:-}"
D8_JAR="${D8_JAR:-$BUILD/r8.jar}"

rm -rf "$CLASSES" "$OUT" "$BUILD/classes.dex"
...
if [[ ! -f "$D8_JAR" ]]; then
  if [[ -f "/root/etg-webview-folders/build/r8.jar" ]]; then
    cp "/root/etg-webview-folders/build/r8.jar" "$D8_JAR"
  else
